...
for nframe = 1:50
    ...
    random_shuffle = randperm(n);
    abort_flag = 0;
    [labels(nframe).positives, labels(nframe).negatives, is_used] = ...
        iterateFrames(permutation(random_shuffle), ...
    ...
    if (length(labels(nframe).negatives.ids) < MAX_N_NEGATIVES)
        ...
        if (neg_start + neg_pool_size - 1 > length(perm_from_other_sequences))
            neg_start = length(perm_from_other_sequences) - neg_pool_size + 1;
        end
        random_shuffle = random_shuffle + neg_start - 1;
